...
import { ShoppingCart, ChevronLeft, Tag, Zap, Plus, Minus } from "lucide-react";
...
const ProductDetail = () => {
  ...
  const { id } = useParams();
  const product = initialProducts.find((data) => data.id == id);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-1">
          ...
          <div className="flex flex-col justify-between">
            ...
            <div className="mt-5 space-y-4 flex justify-center items-center flex-col">
              {cartItem ? (
                <div className="w-full flex items-center justify-between bg-orange-600 rounded-full shadow-lg shadow-orange-800/50 p-2 border-2 border-orange-600">
                  <button
                    onClick={() => removeFromCart(product.id)}
                    className="w-12 h-12 flex items-center justify-center text-white hover:bg-orange-700 rounded-full transition"
                  >
